...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^fusion_") ; then
       FUSION_BUILD=$(echo -n $1 | sed -e 's/^fusion_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $FUSION_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       FUSION_BUILD=
    fi
    export FUSION_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${FUSION_DEVICES_ONLY}" != "z" ]; then
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var FUSION_VERSION)
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.fusion.device=$FUSION_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $FUSION_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $FUSION_FIXUP_COMMON_OUT ]; then
